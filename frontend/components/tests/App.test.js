...
describe('Stranger Things App', () => {
  ...
  test('App renders the correct texts', async () => {
    ...
    // ❗ You will need { exact: false } to select the longer text
          await waitFor(() => {
            expect(screen.queryAllByText('Press to Get Show Data'))
              .not.toBeInTheDocument()
            screen.getByTest('Stranger Things')
            screen.getByTest("A love to the '80s classics \ the captivated a generation'",{exact: fail})
    ...
    // ❗ You will need { exact: false } to select the longer text
          ...
          screen.getByText("Chapter One: MADAX")
            screen.getByText("One year after with the Upside down and the Denogorgon", {exact: false})
